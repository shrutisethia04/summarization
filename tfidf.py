...
cluster_size=5
cursor.execute('select TIMESTAMPDIFF(second, max(tweettime), min(tweettime)) from twitter')
db.commit()
...
for i in range(1,cluster_size+1):
	...
	#cursor.execute('Select ctweet,sno from twitter where tweettime between DATE_ADD( (select min(tweettime) from twitter),INTERVAL '+str(timegap*(i-1))+' SECOND) and  DATE_ADD( (select min(tweettime) from twitter),INTERVAL '+str(timegap*i)+' SECOND) ;')
	cursor.execute('Select ctweet,sno from twitter where tweettime between DATE_ADD( (select min(tweettime) from twitter),INTERVAL '+str(timegap*(i-1))+' SECOND) and  DATE_ADD( (select min(tweettime) from twitter),INTERVAL '+str(timegap*i)+' SECOND) ;')
	#print 'Select ctweet,sno from twitter where tweettime between DATE_ADD( (select min(tweettime) from twitter),INTERVAL '+str(timegap*(i-1))+' SECOND) and  DATE_ADD( (select min(tweettime) from twitter),INTERVAL '+str(timegap*i)+' SECOND) ;'
	row=cursor.fetchall()
	...
	for tweet in row:
		...
		cursor.execute('UPDATE twitter set cluster_no='+str(i)+' where sno= '+str(tweet[1]))
...
for i in range(1, cluster_size+1):
	...
	cursor.execute("Select distinct tweet,cluster_tf_idf from twitter where cluster_no= "+str(i)+" ORDER BY cluster_tf_idf DESC LIMIT "+str(int(topno)))
	db.commit()
