...
cluster_size=5
cursor.execute('select TIMESTAMPDIFF(second, max(tweettime), min(tweettime)) from twitter_duplicate')
db.commit()
...
for i in range(1,cluster_size+1):
	...
	for tweet in row:
		...
		cursor.execute('UPDATE twitter_duplicate set cluster_no='+str(i)+' where sno= '+str(tweet[1]))
...
#print tf_idf_all
scores=[]
update_query= "UPDATE twitter_duplicate SET cluster_tf_idf= CASE sno "
sno_array= "("
for i in tweet_sno:
	cluster= tweet_sno[i]
...
for i in range(1, cluster_size+1):
	...
	cursor.execute("Select distinct tweet,cluster_tf_idf from twitter_duplicate where cluster_no= "+str(i)+" ORDER BY cluster_tf_idf DESC LIMIT "+str(int(topno)))
	db.commit()
